...
import { DATE_TIME_FORMAT, PRIMARY_THEME_COLOR, } from 'app/components/utilities/constant'
                ...
                <FlatList
                    ...
                    onEndReached={() => {
                        if (props?.supportForumList?.length < props?.moreData) {
                            props.getSupportForums(props?.supportForumList?.length > 2 ?
                                props.offSET + 1 : 0, props.filterData)
                        }
                    }}
                    ListEmptyComponent={<EmptyListScreen message={strings.supportForumHeader} />}
                    renderItem={({ item }: any) => {
                        return (
                            <View style={styles.mainView}>
                                <View style={{ paddingHorizontal: 10 }}>
                                    <Text style={styles.dateTxt}>{item?.createdDate ?
                                        moment(item?.createdDate).format(DATE_TIME_FORMAT) : ''}</Text>
